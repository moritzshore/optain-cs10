...
```{r fs_soilmap_intersect, message=FALSE, warning=FALSE}
...
```{r, fs_soilmapplot, fig.cap="Soil map of CS10, in shapefile format", fig.align='center'}
...
```{r, fs_usersoil_preview, echo = FALSE}
datatable(usersoil,
          extensions = "Scroller",
...
```{r, fs_hsgmap, fig.align="center", fig.cap="HSG of CS10"}
...
```{r, fs_join_soil_property}
field_sites_attr <- st_join(cs10_field_sites, soil_propery_map)
```

Here is the result:

```{r, fs_map_joined, echo = FALSE, fig.cap="Field site locations of CS10"}
fs_map <-
  mapview(
    field_sites_attr,
    zcol = "HYDGRP",
...
ws_map <- mapview(basin_shp, alpha.regions = .2)

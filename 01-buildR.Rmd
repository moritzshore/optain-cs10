...
```{r, check_layer_attributes1, eval=run_buildr}
...
```{r, split_land_layer, eval=run_buildr}
...
```{r, channelcheck, eval=run_buildr}
...
```{r, check_cha_res_connectivity, eval=run_buildr}
...
```{r, check_land_drain_ids, eval=run_buildr}
...
```{r, procDEM, message=TRUE, eval=run_buildr}
dem <- rast(dem_path) %>% 
  check_project_crs(layer = ., data_path =  data_path, proj_layer = project_layer, 
...
```{r message=FALSE, warning=FALSE, checksoil1, eval=run_buildr}
...
```{r message=FALSE, warning=FALSE, aggregate_hru_dem_soil, eval=run_buildr}
...
```{r, build_soil_data, eval=run_buildr}
...
```{r, prepare_terrain_land, eval=run_buildr}
...
```{r message=FALSE, warning=FALSE, calculate_land_connectivity, eval=run_buildr, results='hide'}
...
```{r, reduce_land_connections, eval=run_buildr}
...
```{r, resolve_loop_issues, eval=run_buildr}
...
```{r, build_water_object_connectivity, eval=run_buildr}
...
```{r, prepare_water_links, eval=run_buildr}
...
```{r, prepare_terrain_water, eval=run_buildr}
...
```{r, add_wetlands, eval=run_buildr}
...
```{r, build_single_aquifer_files, eval=run_buildr}
...
```{r, add_point_sources, eval=run_buildr}
...
```{r, create_swatplus_database, eval=run_buildr}

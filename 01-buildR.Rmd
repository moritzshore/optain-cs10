...
The OPTAIN project is using the COCOA approach (ref!) schurz2022 and as such, needs to use the `SWATBuildR` package to calculate the connectivity between HRUs in the catchment. This chapter covers this process.
...
require(sf)
require(raster)
require(dplyr) # dplyr must be loaded after SF to avoid conflict
```

If this flag is set to true, the markdown file will actually run buildR, which
...
SWATBuildr reads input data, performs some checks on it, and saves it in
a compatible format for subseqent calculations. Little documentation exists on the creation of these datasets. IF you have any information to add, feel free to do so. 
...
```{r, watershedplot, message=FALSE, echo=FALSE, eval=TRUE}
...
Interactive land use map of CS10 by Field ID
...
```{r, procDEM, message=FALSE, eval=run_code}
dem <- rast(dem_path) %>% 
  check_project_crs(layer = ., data_path =  data_path, proj_layer = project_layer, 

...
```{r, buildr_libs, libraries, message=FALSE}
...
```{r, buildr_plotdemraster, message=FALSE, fig.align = "center", fig.cap="CS10 Digital elevation model (DEM)."}
...
```{r, buildr_watershedplot, message=FALSE, echo=T, eval=TRUE, fig.align='center', fig.cap="CS10 Basin, caclulated from the DEM."}
...
```{r, buildr_maplanduse, message=F, echo=T, fig.align='center', fig.cap="Land use map of CS10 by Farm ID"}
...
```{r, buildr_check_layer_attributes1, eval=run_buildr}
...
```{r, buildr_split_land_layer, eval=run_buildr}
...
```{r buildr_channel_plot_x, fig.align='center', fig.cap="CS10 Channels including surface and subsurface channels"}
...
```{r, buildr_channelcheck, eval=run_buildr}
...
```{r, buildr_check_cha_res_connectivity, eval=run_buildr}
...
```{r, buildr_check_land_drain_ids, eval=run_buildr}
...
```{r, buildr_procDEM, message=TRUE, eval=run_buildr}
dem <- rast(dem_path) %>% 
  check_project_crs(layer = ., data_path =  data_path, proj_layer = project_layer, 
...
```{r buildr_demcropplot, fig.cap="CS10 DEM cropped to basin", message=FALSE, warning=FALSE,  fig.align='center'}
...
```{r, buildr_soil_coverage_rast, message=FALSE, warning=FALSE, eval=run_buildr}
...
```{r buildr_agg_hru_dem_soil, message=FALSE, warning=FALSE, eval=run_buildr}
...
```{r, buildr_build_soil_data, eval=run_buildr}
...
```{r, buildr_prepare_terrain_land, eval=run_buildr}
...
```{r, buildr_calculate_land_connectivity, message=FALSE, warning=FALSE, eval=run_buildr, results='hide'}
...
```{r, buildr_reduce_land_connections, eval=run_buildr}
...
```{r, buildr_infiniteloops, eval = run_buildr}
...
We have decided that at a threshold of 0.3 and 150, we are within the "not that many" loops territory, considering out of 6000+ HRUs, it is only around 2%. Therefore we will let BuildR continue and remove them. This has been discussed in Issue [#55](https://gitlab.nibio.no/moritzshore/swat-cs10/-/issues/55).

```{r, buildr_red_land_con2, eval=run_buildr}
...
```{r, buildr_resolve_loop_issues, eval=run_buildr}
...
```{r, buildr_build_water_object_connectivity, eval=run_buildr}
...
```{r, buildr_prepare_water_links, eval=run_buildr}
...
```{r, buildr_prepare_terrain_water, eval=run_buildr}
...
```{r, buildr_wetland_landuse}
wetland_landuse <- c('wehb', 'wetf', 'wetl', 'wetn')
```

```{r, buildr_add_wetlands, eval=run_buildr}
...
```{r, buildr_build_single_aquifer_files, eval=run_buildr}
...
```{r, buildr_pointsoucemap, fig.align='center', fig.cap="CS10 Point Sources, colored by Phosphorous emission and sized by Nitrogen emission"}
...
```{r, buildr_add_point_sources, eval=run_buildr}
...
```{r, buildr_create_swatplus_database, eval=run_buildr}

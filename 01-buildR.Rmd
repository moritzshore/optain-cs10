...
# not loading any packages because they interfere with the `install_load` 
...
# for the time being i cannot use plot() for rasters. I should switch over to
# ggplot for that
...
lu_map <- ggplot2::ggplot(lu_shp) + ggplot2::geom_sf(aes(fill = type)) + sf_theme +
...
The function `save_dem_slope_raster()` is currently broken, we we are going to
write our own alternative without using Whitebox.

```{r, eval=run_code}
raster::writeRaster(dem, paste0(data_path, '/raster/dem.tif'), 
              overwrite = TRUE)
# wbt_clip_raster_to_polygon replacement
basin <- sf::read_sf(paste0(data_path, '/vector/basin.shp'))
cropped_basin <- terra::crop(dem, basin, mask= T)
#plot(cropped_basin)

raster::writeRaster(cropped_basin,
            paste0(data_path, '/raster/dem2.tif'),
            overwrite = TRUE)

# I am not sure as to what this actually does
# reclass_vals - Reclassification triplet values (new value;from value; to less than)
wbt_reclass(
  input = paste0(data_path, '/raster/dem2.tif'),
  output = paste0(data_path, '/raster/dem3.tif'),
  reclass_vals = "0;9999999;9999999" 
) %>% capture.output(., file = 'NULL')

wbt_slope(
  dem = paste0(data_path, '/raster/dem3.tif'),
...
```{r soilrastermapplot, fig.align='center', fig.cap="CS10 Soil map", message=FALSE, warning=FALSE, echo=F}
...
```{r message=FALSE, warning=FALSE, calculate_land_connectivity, eval=run_code, results='hide'}
...
after writing the txt input files. **This is not implemented in the script
yet.**
...
The point source locations are provided with a point vector layer in the path
'point_path'.
...
Map of point sources, colored by (assumed) phosphorous and size by (assumed)
Nitrogen
...
model from there. These are the steps we have taken, with screenshots since it
is currently not possible to replicate this process in R.
---

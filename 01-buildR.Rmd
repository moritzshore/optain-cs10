...
The function `save_dem_slope_raster()` is currently broken, we we are going to write our own alternative without using Whitebox.
...
Csilla -- What do we do here -- I think in the past Alex just removed all the loops, but it says here it is not recommended when there are a lot of them. Should we increase the threshold? Should we try to break up the loops? You can see the problem polygons on the map below:

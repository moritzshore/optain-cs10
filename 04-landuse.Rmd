...
This section covers the modifications made to the landuse file. For some
reason it was written in a tutorial fashion, as if the reader were new
to R.

```{r lu_libs, message=FALSE}
require(tidyverse)
require(reshape2)
require(sf)
require(DT)
...
\@ref(swatfarmr-management-schedules). Dicussed in Issue [#15](https://gitlab.nibio.no/moritzshore/swat-cs10/-/issues/15)

### Setting `mgt`

This step will be done by SWATFarmR in Section
\@ref(swatfarmr-management-schedules).

Discussed in Issue [#14](https://gitlab.nibio.no/moritzshore/swat-cs10/-/issues/14)
...
```{r lu_urbro_tab, tidy=FALSE, echo=FALSE}
...
```{r lu_urbro_tab2, tidy=FALSE, echo=FALSE}
# how did we do?
...
For the fields we are going to need the crop rotation info which we
created in section \@ref(crop-map-generation).

```{r, lu_croprotationhead}
...
format*](https://r4ds.had.co.nz/tidy-data.html) [@wickham2014]. This
...
`melt` is a function from `reshape2` which is why we `required` it. And
what does that "`.`" mean in there, to the left of `"field"`?? That is a
code word for the object to the left of the "pipe" (in this case
`crop_rotation`). the pipe is "`%>%`" and passes the object to the left
of it, into the function to the right of it. [*look it
up*](https://r4ds.had.co.nz/pipes.html)!
...
uses, which we do not have. We will just use `forest_medium` for all
...
This column has been left as `null` as discussed in Issue [#11](https://gitlab.nibio.no/moritzshore/swat-cs10/-/issues/11)
...
```{r, lu_remove_field_id}
landuse_lum <- landuse_lum %>% dplyr::select(-field_id)
```

lets take a look at the final product:

```{r, lu_preivew_whole_table}
datatable(landuse_lum)
```

Lets write out changes in a new directory where we will store any SWAT+
input files which we have modified from their source BuildR form.

```{r, lu_create_mod_file_dir}
...
But wait -- we need to keep that pesky header, otherwise the FarmR will
...
runs. We will do that with our custom `test_swat_mod()` function. This function
is basically the same as the previous `test_swat()` function, only that it
incorporates the modified SWAT input files

```{r, lu_test_swat, eval = FALSE}
...
Don't write rownames! - [#50](https://gitlab.nibio.no/moritzshore/swat-cs10/-/issues/50)

...
require(sf)
require(DT)
require(dplyr) # require dplyr last to overwrite plyr coun()
```
...
```{r urbro_tab2, tidy=FALSE, echo=FALSE}
# how did we do?
...
format*](https://r4ds.had.co.nz/tidy-data.html) (ref). This format makes it
...
up*](https://r4ds.had.co.nz/pipes.html)! (!ref)
...
But we also need to know where our land uses are... So lets open the file
`SWATbuildR` has prepared for us:

```{r}
lu_map <- "model_data/cs10_setup/optain-cs10/data/vector/land.shp"

lu_map_shp <- read_sf(lu_map)

mapview(lu_map_shp, zcol = "type")
```
...
mapview(soil_depth_map, zcol = "SOL_ZMX")
```
...
mapview(joined_map, zcol = "SOL_ZMX")
```
...
And this is the moment, dear reader... where our coder realized his big
mistake:
...
```{r}
# TODO: remove temp!!!!
cons_prac_path <- "model_data/cs10_setup/temp_cons_practice.lum"
...
This column has already been completed by `SWATbuildR` in (!ref)
...
new_lum_path <- "model_data/cs10_setup/temp_lum_new.lum"

...
```{r, crop_map_lumap, fig.align='center', fig.cap="Land use map for CS10, colored by type."}
...
```{r, crop_map_farmidfig, fig.align='center', fig.cap="Farms in the CS10 catchment, colored by ID"}
...
```{r, crop_map_reported_farms_unique}
...
```{r, crop_map_algorithm, eval=FALSE}
# For every year
for(c_year in years){
  # For every farm,
  for (farm in farms) {
...
```{r, crop_map_exctract_loop, eval=FALSE}
...
```{r, crop_map_croprotgif, fig.align='center', fig.cap="Generated crop map for years 2016-2019", message=FALSE, warning=FALSE, animation.hook='gifski', echo=TRUE}

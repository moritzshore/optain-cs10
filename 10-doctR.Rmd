...
We will create a test directory to run DoctR SWAT runs and copy in our latest
input files from SWATFarmR

```{r}
doctr_path <-  "model_data/swat_doctR/doctr_run"
unlink(doctr_path, recursive = T)

file.copy(
  from = "model_data/cs10_setup/swat_farmR/",
  to = "model_data/swat_doctR/",
  overwrite = T,
  recursive = T
)
file.rename("model_data/swat_doctR/swat_farmR", doctr_path)
...
```{r doc_run_swat_veri_1, eval=FALSE}
...
We will create a directory to store our DoctR induced changes:
```{r}
dir.create("model_data/swat_doctR/swat_input_doc_mod/", showWarnings = F)
```

load Codes.bsn

```{r}
file_path <- "model_data/cs10_setup/swat_farmR/codes.bsn"
codes_df <- read.table(file_path, skip = 1, header = T, sep = "", 
                       colClasses = "character")

```


And enable lapse in codes.bsn
...
```{r}
write_path <- "model_data/swat_doctR/swat_input_doc_mod/codes.bsn"
...
```{r doc_plot_climate, fig.width=8.3, fig.height=11.7, fig.align='center', fig.cap="Climate plot of CS10 generated by SWATDoctR"}
...
```{r, doc_plot_monthly_snow, fig.align='center', fig.cap="Precipitation and Snowmelt on a monthly average basis, as generated by SWATDoctR"}
...
```{r, doc_run_swat_veri_mgt, eval = FALSE}
veri_mgt <-
...
"The function `plot_variable_at_harvkill()` function summarises the state of
variables at the time of harvest/kill operations for all crops in a model setup
and thus provides a general overview"

```{r, eval=FALSE}
plot_variable_at_harvkill()
```

"the function `plot_hru_pw_day()` allows detailed analyses of the daily time
series of HRU related variables, which then can only be performed for a few HRUs
of a model setup."
...
Atmodep not read - [#44](https://gitlab.nibio.no/moritzshore/swat-cs10/-/issues/44)

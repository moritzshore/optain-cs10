# Scheduling Managment with SWATFarmR
...
```{r, cro_gif, echo = FALSE, animation.hook='gifski', fig.cap="Generated crop rotation for CS10."}
...
cs10$read_management(file = "farmR_input/farmR_input2.csv")
```

## Calculating Antecedent Precipitation Index

USING TEMPORARY API: This needs to be replaced with a proper model. See
issue [#17](https://gitlab.nibio.no/moritzshore/swat-cs10/-/issues/17)
...
equation describing the simples version of the model is:
...
Following Python code modified from @patrignani2020 ([[read
more]{.underline}](https://soilwater.github.io/pynotes-agriscience/notebooks/antecedent_precipitation_index.html))

Just do it in R... bokeh wont place nice and we need to do some annoying
time date stuff
...
```{r, eval=FALSE}
cs10$schedule_operations(start_year = 2010, end_year = 2020, n_schedule = 2)
```
...
```{r, eval=FALSE}
cs10$write_operations(start_year = 2010, end_year = 2020)

...
We will extract the landuse file to modify. We also attach the HRU name to be able to search by HRU. Hru-data is loaded as well.
...
`hru2784` (and the within `hru3367`) is a sizable field with landuse a_072f_1 and soil `RGlen-dy-ar`. Tough call here, but best bet is probably to assign soil of the other major field it shares a landuse with (`STlv-sl`)
...
`hru5926` of landuse `a_211f_1` has the wrong soil type (`Sea-dep-thin`), changing to that of the greater field (`STrt-sl`)

```{r, drainfix_5}
hru_fixed$soil[which(hru_fixed$name=="hru5926")] = "STlv-sl"
```

`hru2633` of isolated LU `a_131_f1` can safely have its drains removed.

...
farm_area_map <- mapview(farm_area_sf, zcol = "farm_id")
...
```{r}
field_area_sf <- read_sf("model_data/input/crop/buildr_output_dissolved_into_fields.shp")

field_area_map <- mapview(field_area_sf, zcol = "type")
...
```{r}
 ...
 crop_names <- melt(
  ...
  crop_tidy <- left_join(crop_names, crop_area, by = c("farm_id", "year", "area_daa","pastrure_area_daa", "crop_status")) %>% select(-crop_status)
...
fields <-
  read_csv(
    ...
    show_col_types = F
  ) %>% select(type, farm_id, field_area_daa)

farms <-
  read_csv(
    "model_data/input/crop/buildr_landuse_dissolved_into_farms.csv",
    show_col_types = F
  ) %>% select(type, farm_id , farm_area_)
# fix farm_area_
...
reported_data <- reported_data %>% select(field_id, farm_id, field_area_daa, farm_area_daa)
  ...
  for (farm in farms) {
    ...
    if(crop_filter$farm_id %>% length() == 0){
      ...
      farm_fields <- hru_filter %>%
        select(farm_id, field_id, farm_area_daa, field_area_daa) %>%
    ...
    if((past_area %>% is.na() == FALSE) | has_meadow) {
      ...
      if (length(previously_pasture) > 0) {
        ...
        custom_actual <- farm_fields %>% filter(crop == "meadow") %>%
          select(field_area_daa) %>% pull() %>% sum(na.rm = T)
    ...
    for (c_crop in crop_coverage$crop) {
      crop_index <- which(c_crop ==  crop_coverage$crop)
      crop_coverage$actual[crop_index] <-
        farm_fields %>% filter(crop == c_crop) %>% select(field_area_daa) %>% sum()
    }
...
for(c_field_id in field_ids) {
  ...
  crops <- classed_fields %>% filter(field_id == c_field_id) %>%
    select(crop) %>% t() %>% as_tibble(.name_repair = "minimal")
...
rotation <- crop_rotation %>% select(-field)
...
## Joining the land layer with the crop rotation data

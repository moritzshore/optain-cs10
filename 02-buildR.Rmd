...
If this flag is set to true, the markdown file will actually run buildR, which could take about 4 hours!
...
Land use map of CS10 by Farm ID
...
```{r, channel_plot, fig.cap="CS10 Channels, including surface (cha) and subsurface channels (sub)", fig.align='center'}
channel_path <- 'model_data/input/line/cs10_channels.shp'
channels <- read_sf(channel_path) %>% dplyr::select("type")

channel_map <- ggplot()+geom_sf(data= bound_sf)+geom_sf(data = channels, mapping = aes(color = type))+sf_theme
...
```{r, soilmapraster, echo = F, fig.align='center', fig.cap="CS10 Soil map"}
